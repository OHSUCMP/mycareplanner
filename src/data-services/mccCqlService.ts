...
import {
    mccCodeService,
    mccConditionsLibrary,
    mccGoalsLibrary,
    mccLabResultsLibrary,
    mccMedicationsLibrary,
    mccVitalSignsLibrary
} from './mccCqlLibraries';
...
export const getConditionSummaries = (fhirDataCollection?: FHIRData[]): ConditionSummary[][] | undefined => {
    if (fhirDataCollection === undefined) {
        return undefined
    }
...
const extractMedicationSummary = (fhirData?: FHIRData): MedicationSummary[] | undefined => {
    ...
    console.debug("in extractMedicationSummary.  medicationRequests=" + fhirData.medicationRequests?.length +
        ", medications=" + fhirData.medications?.length);
...
const executeLibrary = (library: any, codeService: any, patientSource: any): any => {
    try {
        ...
        console.debug("executeLibrary: extractedSummary=", JSON.stringify(extractedSummary));

        return extractedSummary
    } catch (err) {
        console.error("Error executing CQL: " + err)
        return undefined
    }
}
